...
<main>
    ...
    @endif
      <div class="single-product__img w-100" id="main-img">
        <img src="{{$product->image[0]=='h' ? $product->image : asset('uplode/Book') . '/' . $product->image   }}" alt="">
      </div>
        ...
        <div>
          ...
          <div class="product__price mb-3 text-center d-flex gap-2">
            <span class="product__price product__price--old fs-6 ">
                {{ $product->price * $qun }} جنيه
            </span>
            <span class="product__price fs-5">
                {{ $product->price_after_discount * $qun }} جنيه
            </span>
          </div>
          <div class="d-flex w-100 gap-2 mb-3">
            <div class="single-product__quanitity position-relative">
              ...
              {{-- <div class="pt-3"> --}}
                {{-- @dd($qun) --}}
                  <a href="{{ route('home.inc',['id'=>$product->id ,'qun'=>$qun] ) }}" style="font-size: 30px; cursor: pointer;" class="single-product__increase border-0  text-decoration-none text-secondary  bg-transparent position-absolute end-0 h-100 px-3">+</a>

                  <a href="{{ route('home.dec',['id'=>$product->id ,'qun'=>$qun] ) }}" style="font-size: 30px; cursor: pointer;" class="single-product__decrease border-0  text-decoration-none  text-secondary bg-transparent position-absolute start-0 h-100 px-3">-</a>
                  <h4 class="single-product__input text-center pt-2 px-3">{{ $qun }}</h4>
                </div>
            {{-- </div> --}}
            @auth
<a href="{{ route('home.addToCart',['id'=>$product->id,'qun'=>$qun]) }}" class="text-white text-decoration-none">
    ...
    @endauth
          ...
          <div class="single-product__favourite d-flex align-items-center gap-2 mb-4">
            ...
            <a style="cursor: pointer;" href="{{ route('home.favourite',['id'=>$product->id  ]) }}">
                <div class="single-product__favourite d-flex align-items-center gap-2 mb-4">
                    <i class="fa-regular fa-heart"></i>
                    اضافة للمفضلة
                  </div>
                ...
                <label class="form-check-label" for="flexSwitchCheckDefault">            اضافة للمفضلة                </label>
            </div> --}}
            {{-- @if ($fav==0)
            <i class="fa-regular fa-heart"></i>
            @endif
        ...
        <div class="single-product__categories">
          ...
          <div>
            <span>التصنيفات: </span><a href="{{ route('shop.index') }}">{{ $category->name }}</a>
          </div>
